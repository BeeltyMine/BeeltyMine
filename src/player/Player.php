...
class Player extends Human implements CommandSender, ChunkListener, IPlayer, NeverSavedWithChunkEntity
{
	...
	protected function orderChunks(): void
	{
		...
		foreach (
			$this->chunkSelector->selectChunks(
	...
	protected function internalSetGameMode(GameMode $gameMode): void
	{
		...
		$this->allowFlight = $this->gamemode === GameMode::CREATIVE || $this->gamemode === GameMode::SPECTATOR;
		...
		if ($this->isSpectator()) {
			...
			// Close any open windows/forms when entering spectator so the client UI is clean
	...
	{
		return !$this->isSpectator() && parent::isFreezable();
	}
}
